...
int main(int argc, char **argv)
{
  ...
  setup_ros_publishers(node_handler, image_transport, sensor_type);
    ...
    tf2_ros::TransformListener listener(buffer,node_handler);


  while(1){
    std::this_thread::sleep_for(std::chrono::milliseconds(50));
    if(init_transform_world(buffer)) break;
  }
...
void ImageGrabber::SyncWithImu()
{
    ...
    while(1)
    {
        ...
        if (!imgLeftBuf.empty()&&!imgRightBuf.empty()&&!mpImuGb->imuBuf.empty())
        {
          ...
          bool valid_origin_pose = false;
          Sophus::SE3f transform_map_origin = mpSLAM->GetPoseOriginCurrentMap(valid_origin_pose);

          auto Twc = Tcw.inverse();

          auto P = transform_to_world * ( transform_cam_to_link * (transform_map_origin * Twc) * transform_cam_to_link.inverse());


          publish_ros_camera_pose(P, msg_time);
          publish_ros_marker(P, msg_time);
          ...
          ikkk+=1;
          if(ikkk % 100 == 0){
            mpSLAM->SaveTrajectoryTUM("/tmp/traj.txt");
            mpSLAM->SaveKeyFrameTrajectoryTUM("/tmp/trajkf.txt");
          }
        }
    }
}
